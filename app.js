...
async function updateEmployeeRole() {
    ...
    inquirer.prompt([
        ...
        if(answers.empName != "Cancel") {
            ...
            db.query("UPDATE employee SET role_id=? WHERE id=?", [roleID, empName]);
    ...
    inquirer.prompt([
        ...
        inquirer.prompt([
            ...
            },
            {
                name: "roleDepartment",
                type: "list",
                message: departments.map(obj => obj.name)
            }
        ]).then(answers => {
            let depID = departents.find(obj => obj.name === answers.roleDepartment).id
            let roleId = roles.find(obj => obj.title === responce.roleName).id

            db.query("UPDATE role SET title=?, salary=?, department_id=? WHERE id=?", [responce.roleName, answers.salaryNum, depID, roleID]);
